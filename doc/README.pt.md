...
Reproduz o som `snd` (0-63) no canal `ch` (0-3). Se `snd` é uma lista, os sons serão reproduzidos em ordem. O número da nota para iniciar a reprodução pode ser especificado com `pos`. Se `True` for especificado para `loop`, a reprodução será feita em laço.

- `playm(msc, [pos], [loop])`<br>
Reproduz a música `msc` (0-7). O número da nota para iniciar a reprodução pode ser especificado com `pos`. Se `True` for especificado para `loop`, a reprodução será feita em laço.
