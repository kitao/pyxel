...
채널 `ch` (0-3)에서 사운드 `snd` (0-63)를 재생합니다. `snd`가 리스트일 경우, 순서대로 재생됩니다. 재생을 시작할 음표 번호는 `pos`로 지정할 수 있습니다. `loop`에 `True`를 지정하면 계속 반복합니다.

- `playm(msc, [pos], [loop])`<br>
음악 `msc` (0-7)을 재생합니다. 재생을 시작할 음표 번호는 `pos`로 지정할 수 있습니다. `loop`에 `True`를 지정하면 계속 반복합니다.
