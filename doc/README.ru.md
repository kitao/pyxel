...
Проиграть звук `snd` (0-63) на канале `ch` (0-3). Если `snd` — список, он будет проигран по порядку. Номер ноты для начала воспроизведения можно указать с помощью `pos`. Если в в качестве значения `loop` передано `True`, проигрывание будет зациклено.

- `playm(msc, [pos], [loop])`<br>
Проиграть трек `msc` (0-7). Номер ноты для начала воспроизведения можно указать с помощью `pos`. Если в в качестве значения `loop` передано `True`, проигрывание будет зациклено.
