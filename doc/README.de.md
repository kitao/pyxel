...
Spielt den Ton `snd` (0-63) auf Kanal `ch` (0-3). Falls `snd` eine Liste ist, wird es in der Reihenfolge abgespielt. Die Notennummer zum Starten der Wiedergabe kann mit `pos` angegeben werden. Falls `True` für `loop` angeben ist, wird eine Schleifenwiedergabe durchgeführt.

- `playm(msc, [pos], [loop])`<br>
Spielt die Musik `msc` (0-7). Die Notennummer zum Starten der Wiedergabe kann mit `pos` angegeben werden. Falls `True` für `loop` angegeben ist, wird eine Schleifenwiedergabe durchgeführt.
