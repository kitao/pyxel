...
チャンネル`ch` (0-3) でサウンド`snd` (0-63) を再生します。`snd`がリストの場合順に再生されます。再生を開始するノート番号を`pos`で指定できます。`loop`に`True`を指定するとループ再生します。

- `playm(msc, [pos], [loop])`<br>
ミュージック`msc` (0-7) を再生します。再生を開始するノート番号を`pos`で指定できます。`loop`に`True`を指定するとループ再生します。
