...
_pyxel_pal = pyxel.pal


def _pal(src_color=None, dst_color=None):
    if src_color or dst_color:
        _pyxel_pal(src_color, pyxel.NUM_COLORS + dst_color)
        _pyxel_pal(src_color, pyxel.NUM_COLORS + dst_color)
    else:
        for i in range(pyxel.NUM_COLORS):
            _pyxel_pal(i, pyxel.NUM_COLORS + i)
