...
| Command | Description |
| --- | --- |
| `T <bpm>` | Sets the tempo (BPM). Range: 1 or greater. Default is 120. |
...
| `@ <tone>` | Sets the tone. The initial tones are 0:Triangle / 1:Square / 2:Pulse / 3:Noise. Default is 0. |
...
| `@ENV <slot>` | Switches the envelope (volume curve) slot. Range: 0 or greater. 0 turns it off. |
| `@ENV <slot> { init_vol, dur_ticks1, vol1, dur_ticks2, vol2, ... }` | Slot number: 1 or greater. Sets and switches to the specified envelope slot. Slot 0 cannot be specified. Inside `{ }`, specify "initial volume (once)", then repeat "duration (tick), volume (vol)". 1 tick is 1/48 of a quarter note. Example: `@ENV1 { 30, 20, 100, 50, 0 }` |
| `@VIB <slot>` | Switches the vibrato (pitch modulation) slot. Range: 0 or greater. 0 turns it off. |
