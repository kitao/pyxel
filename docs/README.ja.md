...
公式のインストーラーで Python をインストールする場合は、`pyxel`コマンドを有効にするために、`Add Python 3.x to PATH`にチェックを入れて Python のインストールを行なってください。
...
指定した Pyxel リソースファイル (.pyxres) が存在する場合は読み込み、存在しない場合は指定した名前で新規にファイルを作成します。リソースファイルを省略した場合は`my_resource.pyxres`というファイル名になります。
...
起動スクリプトに次の形式でメタデータを記載すると実行時に表示できます。`title`、`author`以外のフィールドは省略可能です。
...
Pyxel は[MIT ライセンス](../LICENSE)です。ソースコードやライセンス表示用のファイル等で、著作権とライセンス全文の表示を行えば、自由に販売や配布をすることができます。
