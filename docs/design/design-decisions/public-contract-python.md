...
preserving it requires only changing the operation order. The parser and loader already return the replacement data, so preparing it
first avoids data loss without backup or restoration machinery.
...
later sequences. Preserving all supplied data also agrees with direct sequence editing and
resource loading.
