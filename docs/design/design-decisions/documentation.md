...
| Chinese | Half-width parentheses with a half-width space against adjacent Chinese text, as `scripts/format_prose` applies; range mark ～; instructions to the reader take the bare imperative for operating steps and 请 + verb for requests; 屏幕 for the Pyxel screen (the drawing target and its size, including the `pyxel-screen` element) and for the device display, 画面 for a page, panel, or scene of a tool or game, including the Pyxel Editor window; 瓦片 for a tile | Parentheses and spacing are the maintainer's tool-applied choice; GB/T 15834 for the range mark; terms are maintainer choices |
