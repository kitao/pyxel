...
Après avoir installé le paquet SDL2 (`libsdl2-dev` pour Ubuntu), [Python 3](https://www.python.org/) (version 3.8 ou supérieure) et `python3-pip`, exécutez la commande suivante :
...
- `gen_bgm(preset, transp, instr, [seed], [play])`<br>
  Crée une liste MML de BGM avec un algorithme basé sur [8bit BGM generator](https://github.com/shiromofufactory/8bit-bgm-generator). `preset` est le numéro de preset (0-7), `transp` la hauteur (-5 à 6) et `instr` le numéro d’instrumentation (0-3) : `0`=mélodie+réverb+basse, `1`=mélodie+basse+batterie, `2`=mélodie+sub+basse, `3`=mélodie+sub+basse+batterie. Si `seed` n’est pas spécifié, le résultat est aléatoire. Si `play` vaut `True`, le MML créé est joué.
