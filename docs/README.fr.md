...
- `pget(x, y)`<br>
  Obtient la tuile à (`x`, `y`). Une tuile est représentée sous forme de tuple `(tile_x, tile_y)`.

- `pset(x, y, tile)`<br>
  Dessine une `tuile` à (`x`, `y`). Une tuile est représentée sous forme de tuple `(tile_x, tile_y)`.
