...
Крім того, команда `pyxel watch` відстежує зміни у вказаному каталозі і автоматично перезапускає програму, якщо виявлено будь-які зміни.

```sh
pyxel watch WATCH_DIR PYTHON_SCRIPT_FILE
```

Цю команду можна зупинити за допомогою комбінації клавіш `Ctrl(Command)+C`.
