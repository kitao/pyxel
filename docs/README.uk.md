...
<td>Малювання користувацького шрифта</td>
<td><a href="https://kitao.github.io/pyxel/wasm/examples/13-custom-font.html">Демонстрація</a></td>
...
<td>Малювання Tiled Map File (.tmx) та автоматична генерація BGM</td>
...
<td>Запускач застосунків Pyxel (ви можете грати в різні ігри!)</td>
<td><a href="https://kitao.github.io/pyxel/wasm/examples/17-app-launcher.html">Демонстрація</a></td>
...
<td>Відтворення аудіо</td>
<td><a href="https://kitao.github.io/pyxel/wasm/examples/18-audio-playback.html">Демонстрація</a></td>
...
- `colors`<br>
  Список кольорів палітри. Кольори відображення вказуються у 24-бітному числовому значенні. Працює як список Python: кольори можна додавати, видаляти або замінювати масово.<br>
  Приклад: `old_colors = list(pyxel.colors); pyxel.colors[:] = [0x111111, 0x222222, 0x333333]; pyxel.colors[15] = 0x112233`
...
- `musics`<br>
  Список музичних треків (екземпляри класу Music) (0-7)
...
- `gen_bgm(preset, instr, [seed], [play])`<br>
  Генерує список MML для BGM за алгоритмом, заснованим на [8bit BGM generator](https://github.com/shiromofufactory/8bit-bgm-generator). `preset` — номер пресету (0-7), `instr` — номер складу (0-3): `0`=мелодія+реверб+бас, `1`=мелодія+бас+ударні, `2`=мелодія+саб+бас, `3`=мелодія+саб+бас+ударні. Якщо `seed` не задано, результат випадковий. Якщо `play` дорівнює `True`, відтворюється згенерований MML.
...
- `clamp(x, lower, upper)`<br>
  Повертає `x`, обмежене між `lower` як мінімальним значенням і `upper` як максимальним значенням.
...
- `rseed(seed)`<br>
  Встановлює початкове зерно генератора випадкових чисел.
...
- `nseed(seed)`<br>
  Встановлює початкове зерно для шуму Перліна.
...
- `collide(x, y, w, h, dx, dy, walls)`<br>
  Розв'язує зіткнення після застосування переміщення (`dx`, `dy`) до прямокутника в позиції (`x`, `y`) з розміром (`w`, `h`) і повертає скориговані (`dx`, `dy`). `walls` — це список тайлів `(image_tx, image_ty)`, які вважаються стінами.
...
- `pcm(filename)`<br>
  Завантажує аудіофайл (WAV/OGG) для відтворення. Викличте `pcm()`, щоб повернутися до звичайного режиму відтворення.<br>
  Приклад: `pyxel.sounds[0].pcm("sounds/bgm.ogg")`
