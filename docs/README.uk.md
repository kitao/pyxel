...
- `pget(x, y)`<br>
  Повертає тайл на позиції (`x`, `y`). Тайл представлений у вигляді кортежу `(tileset_x, tileset_y)`.

- `pset(x, y, tile)`<br>
  Малює `tile` на позиції (`x`, `y`). Тайл представлений у вигляді кортежу `(tileset_x, tileset_y)`.
