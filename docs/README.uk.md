...
- `gen_bgm(preset, transp, instr, [seed], [play])`<br>
  Створює список MML для BGM за алгоритмом, заснованим на [8bit BGM generator](https://github.com/shiromofufactory/8bit-bgm-generator). `preset` — номер пресету (0-7), `transp` — висота тону (-5 до 6), `instr` — номер складу (0-3): `0`=мелодія+реверб+бас, `1`=мелодія+бас+ударні, `2`=мелодія+саб+бас, `3`=мелодія+саб+бас+ударні. Якщо `seed` не задано, результат випадковий. Якщо `play` дорівнює `True`, відтворюється створений MML.
