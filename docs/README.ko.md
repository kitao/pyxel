...
Mac에 기본적으로 설치되는 Python3를 사용하는 경우 위 명령의 시작 부분에 `sudo`를 추가하여 `pyxel` 명령을 활성화하십시오.
...
숫자 키를 누른 상태에서 TMX 파일 (Tiled Map File) 을 타일맵 편집기 화면으로 끌어서 놓으면 지정된 인덱스 번호를 가진 레이어를 현재 선택된 타일맵에 로드할 수 있습니다.
