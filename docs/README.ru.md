...
- `pget(x, y)`<br>
  Получает тайл в координатах (`x`, `y`). Тайлы представлены в виде кортежа `(tileset_x, tileset_y)`.

- `pset(x, y, tile)`<br>
  Рисует `tile` в координатах (`x`, `y`). Тайл представлен в виде кортежа `(tileset_x, tileset_y)`.
