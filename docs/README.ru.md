...
Перетащите файл TMX (Tiled Map File) в редактор тайловых карт, чтобы загрузить его слой в порядке отрисовки, соответствующем текущему выбранному номеру тайловой карты.
...
Режим для редактирования musics, в которых звуки расположены в порядке воспроизведения.
...
- `pget(x, y)`<br>
  Получает тайл в координатах (`x`, `y`). Тайлы представлены в виде кортежа `(tile_x, tile_y)`.

- `pset(x, y, tile)`<br>
  Рисует `tile` в координатах (`x`, `y`). Тайл представлен в виде кортежа `(tile_x, tile_y)`.
