...
- `gen_bgm(preset, instr, [seed], [play])`<br>
  Создаёт список MML для BGM с алгоритмом на основе [8bit BGM generator](https://github.com/shiromofufactory/8bit-bgm-generator). `preset` — номер пресета (0-7), `instr` — номер состава (0-3): `0`=мелодия+реверб+бас, `1`=мелодия+бас+ударные, `2`=мелодия+саб+бас, `3`=мелодия+саб+бас+ударные. Если `seed` не задан, результат случайный. Если `play` равно `True`, воспроизводится созданный MML.
