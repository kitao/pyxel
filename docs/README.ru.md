...
Pyxel имеет открытый исходный код под [лицензией MIT](../LICENSE) и свободен для использования. Давайте начнем создавать ретро-игру с помощью Pyxel!
...
- Редактор изображений и звуков
- Возможность расширения палитры, каналов и банков пользователем
...
Для получения конкретных инструкций, пожалуйста, обратитесь к [этой странице](pyxel-web-en.md).
...
<td>1-я победная игра Pyxel Jam от <a href="https://x.com/helpcomputer0">Adam</a></td>
...
<td>Аркадная игра с физикой мяча от <a href="https://x.com/helpcomputer0">Adam</a></td>
...
<td>Генератор фоновой музыки, созданный <a href="https://x.com/frenchbread1222">frenchbread</a></td>
...
Кроме того, команда `pyxel watch` позволяет отслеживать изменения в указанном каталоге, автоматически перезапуская программу при обнаружении изменений:

```sh
pyxel watch WATCH_DIR PYTHON_SCRIPT_FILE
```

Мониторинг директорий можно остановить, нажав `Ctrl(Command)+C`.
...
- `play_pos(ch)`<br>
  Получить позицию канала `ch`(0-3) в виде кортежа `(sound_no, note_no)`. Возвращает `None` если проигрывание выключено.
...
- `sgn(x)`<br>
  Возвращает `1`, если `x` положительно, `0`, если оно равно `0`, и `-1`, если оно отрицательно.
