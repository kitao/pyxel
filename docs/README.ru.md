...
Кроме того, команда `pyxel watch` отслеживает изменения в указанном каталоге и автоматически перезапускает программу при обнаружении каких-либо изменений.

```sh
pyxel watch WATCH_DIR PYTHON_SCRIPT_FILE
```

Эту команду можно остановить, нажав `Ctrl(Command)+C`.
