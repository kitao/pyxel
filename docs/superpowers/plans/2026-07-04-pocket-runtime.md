...
**Current implementation status:** The PocketPy module now registers all public paths from `python/pyxel/__init__.pyi`, including core-backed `Font`, `Image`, `Tilemap`, `Channel`, `Tone`, `Sound`, and `Music` wrappers plus the global object collections. Wrapper userdata stores cloned `Rc*` handles from `pyxel-core`, so constructors and global collections operate on real core objects instead of presence-only stubs. Runtime smoke coverage now exercises Python-facing sequence behavior for `colors`, global object collections, `Sound` integer sequences, `Tone.wavetable`, and `Music.seqs`, including iteration, negative indexing, `append`, `extend`, `insert`, `pop`, `del`, containment, equality against lists, and truthiness. `data_ptr()` currently returns the backing address as an integer because PocketPy does not provide CPython `ctypes`; exact CPython-compatible buffer exposure remains a separate compatibility decision.
