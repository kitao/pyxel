...
fn main() {
    ...
    // Generate editor module embedding
    let editor_base = format!("{manifest_dir}/../../python/pyxel/editor");
    let mut entries = Vec::new();

    for (subdir, mod_prefix) in [("", "pyxel.editor"), ("widgets", "pyxel.editor.widgets")] {
        let dir = if subdir.is_empty() {
            editor_base.clone()
        } else {
            format!("{editor_base}/{subdir}")
        };
        ...
        for file in &files {
            ...
            let mod_name = if stem == "__init__" {
                mod_prefix.to_string()
            } else {
                format!("{mod_prefix}.{stem}")
            };
            entries.push((mod_name, file.to_string_lossy().to_string()));
        }
    }

    let mut codegen = String::from(
        "/// Auto-generated by build.rs — do not edit.\n\
         pub const EDITOR_MODULES: &[(&str, &str)] = &[\n",
    );
    for (mod_name, abs_path) in &entries {
        codegen.push_str(&format!(
            "    (\"{mod_name}\", include_str!(\"{}\")),\n",
            abs_path.replace('\\', "/")
        ));
    }
    codegen.push_str("];\n");

    let out_path = Path::new(&out_dir).join("editor_modules.rs");
    fs::write(&out_path, codegen).unwrap();

    // Rebuild when editor sources change
    println!("cargo:rerun-if-changed={editor_base}");
}
