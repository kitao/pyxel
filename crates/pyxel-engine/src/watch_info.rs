...
impl WatchInfo {
    pub fn new() -> Self {
        ...
        let (mut window_x, mut window_y) = pyxel_platform::window_pos();
        let (mut window_width, mut window_height) = pyxel_platform::window_size();
        if let Some(ref watch_info_file) = watch_info_file {
            ...
            if watch_info.len() == 4 {
                window_x = watch_info[0].parse::<i32>().unwrap();
                window_y = watch_info[1].parse::<i32>().unwrap();
                window_width = watch_info[2].parse::<u32>().unwrap();
                window_height = watch_info[3].parse::<u32>().unwrap();
