...
mod tests {
    ...
    fn new_channel() -> &'static mut Channel {
        unsafe { &mut *Channel::new() }
    }
    ...
    fn test_channel_initial_state() {
        ...
        assert!(ch.sounds.is_empty());
        assert!(ch.owned_sounds.is_empty());
    }

    #[test]
    fn test_sec_to_clock() {
        // None maps to 0
        assert_eq!(Channel::sec_to_clock(None), 0);
        // 0.0 seconds maps to 0 clocks
        assert_eq!(Channel::sec_to_clock(Some(0.0)), 0);
        // 1.0 second maps to AUDIO_CLOCK_RATE clocks
        assert_eq!(Channel::sec_to_clock(Some(1.0)), AUDIO_CLOCK_RATE);
    }

    #[test]
    fn test_play_mml_sets_playing() {
        crate::test_init();
        let ch = new_channel();
        ch.play_mml("T120L4CDEF", None, false, false).unwrap();
        assert!(ch.is_playing);
        assert_eq!(ch.owned_sounds.len(), 1);
    }

    #[test]
    fn test_stop_clears_owned_sounds() {
        // Regression test for C7: owned sounds must be freed on stop
        crate::test_init();
        let ch = new_channel();
        ch.play_mml("T120L4CDEF", None, false, false).unwrap();
        assert_eq!(ch.owned_sounds.len(), 1);
    ...
    #[test]
    fn test_play_mml_releases_previous() {
        // Regression test for C7: successive play_mml calls must not accumulate sounds
        crate::test_init();
        let ch = new_channel();
        ch.play_mml("T120L4CDEF", None, false, false).unwrap();
        assert_eq!(ch.owned_sounds.len(), 1);
        ch.play_mml("T120L4GABC", None, false, false).unwrap();
        assert_eq!(ch.owned_sounds.len(), 1);
    }
}
