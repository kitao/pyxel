...
mod tests {
    ...
    fn new_tilemap(w: u32, h: u32) -> &'static mut Tilemap {
        unsafe { &mut *Tilemap::new(w, h, ImageSource::Index(0)) }
    }

    #[test]
    fn test_tilemap_new_dimensions() {
        crate::test_init();
        let tm = new_tilemap(16, 8);
    ...
    fn test_tilemap_set_get_tile() {
        crate::test_init();
        let tm = new_tilemap(8, 8);
        tm.set_tile(3.0, 4.0, (5, 12));
        assert_eq!(tm.get_tile(3.0, 4.0), (5, 12));
    }

    #[test]
    fn test_tilemap_clear() {
        crate::test_init();
        let tm = new_tilemap(4, 4);
        tm.clear((3, 7));
        for y in 0..4 {
            for x in 0..4 {
                assert_eq!(tm.get_tile(x as f32, y as f32), (3, 7));
            }
        }
    }

    #[test]
    fn test_tilemap_default_tile_is_zero() {
        crate::test_init();
        let tm = new_tilemap(4, 4);
        for y in 0..4 {
            for x in 0..4 {
                assert_eq!(tm.get_tile(x as f32, y as f32), (0, 0));
            }
        }
    }
}
