...
mod tests {
    ...
    // Build a minimal BDF font in memory for testing
    fn make_bdf_font(glyphs: Vec<(i32, BdfGlyph)>) -> Font {
        Font::Bdf {
            bounding_box: BdfBoundingBox {
                width: 4,
                height: 6,
    ...
    fn simple_glyph(code: i32, dwidth: i32) -> (i32, BdfGlyph) {
        (
            code,
            BdfGlyph {
                dwidth,
    ...
    fn test_is_invisible() {
        // Control characters
        assert!(Font::is_invisible('\0'));
        assert!(Font::is_invisible('\t'));
        assert!(Font::is_invisible('\r'));

        // Variation selectors
        assert!(Font::is_invisible('\u{FE00}'));
        assert!(Font::is_invisible('\u{FE0F}'));

        // Bidi controls
        assert!(Font::is_invisible('\u{200E}'));
        assert!(Font::is_invisible('\u{200F}'));

        // Zero-width specials
        assert!(Font::is_invisible('\u{200B}')); // ZWSP
        assert!(Font::is_invisible('\u{200D}')); // ZWJ
        assert!(Font::is_invisible('\u{2060}')); // Word joiner

        // Visible characters
        assert!(!Font::is_invisible('A'));
        assert!(!Font::is_invisible(' '));
        assert!(!Font::is_invisible('あ'));
        assert!(!Font::is_invisible('0'));
    }

    #[test]
    fn test_text_width_single_line() {
        // 'A'=65, 'B'=66, 'C'=67, each with dwidth=4
        let mut font = make_bdf_font(vec![
            simple_glyph('A' as i32, 4),
            simple_glyph('B' as i32, 4),
            simple_glyph('C' as i32, 4),
        ]);
        assert_eq!(font.text_width("ABC"), 12);
        assert_eq!(font.text_width("A"), 4);
        assert_eq!(font.text_width(""), 0);
    }

    #[test]
    fn test_text_width_multiline() {
        let mut font = make_bdf_font(vec![
            simple_glyph('A' as i32, 4),
            simple_glyph('B' as i32, 6),
        ]);
        // "AB\nA" => line1=10, line2=4 => max=10
        assert_eq!(font.text_width("AB\nA"), 10);
        // "A\nAB" => line1=4, line2=10 => max=10
        assert_eq!(font.text_width("A\nAB"), 10);
    }

    #[test]
    fn test_text_width_skips_invisible() {
        let mut font = make_bdf_font(vec![
            simple_glyph('H' as i32, 5),
            simple_glyph('i' as i32, 3),
        ]);
        // Invisible chars (ZWJ, variation selector) should not add width
        assert_eq!(font.text_width("Hi"), 8);
        assert_eq!(font.text_width("H\u{200D}i"), 8);
        assert_eq!(font.text_width("H\u{FE0F}i"), 8);
    }
}
