...
mod tests {
    ...
    fn new_image(w: u32, h: u32) -> &'static mut Image {
        unsafe { &mut *Image::new(w, h) }
    }

    #[test]
    fn test_image_new_dimensions() {
        crate::test_init();
        let img = new_image(32, 16);
        assert_eq!(img.width(), 32);
        assert_eq!(img.height(), 16);
    }

    #[test]
    fn test_image_pset_pget() {
        crate::test_init();
        let img = new_image(8, 8);
        img.set_pixel(3.0, 4.0, 5);
        assert_eq!(img.get_pixel(3.0, 4.0), 5);
    }

    #[test]
    fn test_image_clear() {
        crate::test_init();
        let img = new_image(4, 4);
        img.clear(7);
        for y in 0..4 {
            for x in 0..4 {
                assert_eq!(img.get_pixel(x as f32, y as f32), 7);
            }
        }
    }

    #[test]
    fn test_image_pget_out_of_bounds() {
        crate::test_init();
        let img = new_image(8, 8);
        img.set_pixel(0.0, 0.0, 3);
        // Out-of-bounds reads return 0
        assert_eq!(img.get_pixel(-1.0, 0.0), 0);
        assert_eq!(img.get_pixel(0.0, -1.0), 0);
        assert_eq!(img.get_pixel(8.0, 0.0), 0);
        assert_eq!(img.get_pixel(0.0, 8.0), 0);
    }

    #[test]
    fn test_image_blt_copy() {
        crate::test_init();
        let src = new_image(8, 8);
        src.set_pixel(2.0, 3.0, 9);
        src.set_pixel(4.0, 5.0, 11);

        let dst = new_image(8, 8);
        unsafe {
            dst.draw_image(
                0.0,
                0.0,
        ...
        assert_eq!(dst.get_pixel(2.0, 3.0), 9);
        assert_eq!(dst.get_pixel(4.0, 5.0), 11);
