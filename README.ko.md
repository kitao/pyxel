...
`caption`으로 창 제목, `scale`로 표시 배율, `palette`로 팔레트 색, `fps`로 동작 프레임 레이트, `quit_key`로 애플리케이션의 종료 키를 지정할 수 있습니다, and whether to start in full screen with `fullscreen`. `palette`는 24비트 색상 중 16개의 요소로 지정합니다.<br>
