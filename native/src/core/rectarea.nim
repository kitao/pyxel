...
proc initRectAreaFromPos*(x1, y1, x2, y2: int): RectArea =
  if x1 < x2:
    result.left = x1
    result.right = x2
...
proc initRectAreaFromSize*(left, top, width, height: int): RectArea =
...
proc intersects*(self: RectArea, rect: RectArea): Option[RectArea] {.inline.} =
  ...
  if width <= 0 or height <= 0:
    result = none(RectArea)
  else:
    result = some(RectArea(left: left, top: top, right: right, bottom: bottom,
