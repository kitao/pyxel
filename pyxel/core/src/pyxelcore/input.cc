...
void Input::Update(Window* window, int32_t frame_count) 
{
  ...
  mouse_x_ = (mouse_x_ - (window->WindowX() + window->ScreenX())) / window->ScreenScale();
  mouse_y_ = (mouse_y_ - (window->WindowY() + window->ScreenY())) / window->ScreenScale();
  mouse_wheel_ = window->GetMouseWheel();

  if (is_mouse_visible_) 
  {
    SDL_ShowCursor(true);
    SDL_SetCursor(mouse_x_ >= 0 && mouse_x_ < window->ScreenWidth() && mouse_y_ >= 0 && mouse_y_ < window->ScreenHeight()  ?  blank_cursor_ : normal_cursor_);
  } 
  else 
  {
  ...
  UpdateKeyState(KEY_SHIFT, GET_KEY_STATE(KEY_LEFT_SHIFT) || GET_KEY_STATE(KEY_RIGHT_SHIFT));

  UpdateKeyState(KEY_CONTROL, GET_KEY_STATE(KEY_LEFT_CONTROL) || GET_KEY_STATE(KEY_RIGHT_CONTROL));

  UpdateKeyState(KEY_ALT, GET_KEY_STATE(KEY_LEFT_ALT) || GET_KEY_STATE(KEY_RIGHT_ALT));

  UpdateKeyState(KEY_SUPER, GET_KEY_STATE(KEY_LEFT_SUPER) || GET_KEY_STATE(KEY_RIGHT_SUPER));
  ...
  if (gamepad1_) 
  {
    ...
    {
      UpdateKeyState(GAMEPAD_1_A + i, SDL_GameControllerGetButton(gamepad1_, SDL_BUTTON_TABLE[i]));
    }
  }
  ...
  if (gamepad2_) 
  {
    for (int32_t i = 0; i < BUTTON_COUNT; i++) 
    {
      UpdateKeyState(GAMEPAD_2_A + i, SDL_GameControllerGetButton(gamepad2_, SDL_BUTTON_TABLE[i]));
    }
  }
}
...
bool Input::IsButtonPressed(int32_t key, int32_t hold_frame,int32_t period_frame) const 
{
  if (key < 0 || key >= KEY_COUNT) 
  {
    PYXEL_ERROR("invalid key");
