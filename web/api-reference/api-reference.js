const PYXEL_API_REF_ADVANCED_KEY = "pyxel-api-ref-advanced";
...
function buildPage() {
  ...
  toggleCheck.addEventListener("change", () => {
    ...
    localStorage.setItem(PYXEL_API_REF_ADVANCED_KEY, showAdvanced ? "1" : "0");
    updateVisibility();
  });
  ...
  for (const cat of data.categories) {
    ...
    if (cat.items && cat.items.length > 0) {
      ...
      cardTitle.className = "font-semibold text-xl text-gray-100 mb-3";
      cardTitle.dataset.catTitle2 = cat.id;
        ...
        if (group.sections && group.sections.length > 0) {
          ...
          for (let s = 0; s < group.sections.length; s++) {
            ...
            if (sec.details && sec.details.length > 0) {
              ...
              list.className = "mt-2 pl-3 border-l-2 border-gray-700 space-y-1";
